...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            target_words = [words.split() if words.split() else ["EMPTY"] for words in batch.trn]
...
def dataio_prepare(hparams):
    ...
    testdata = (
            wds.WebDataset(hparams["testshards"])
            .decode()
            .rename(trn="transcript.txt", wav="audio.pth")
            .map(tokenize)
            .batched(
    ...
    return {"train": traindata, "valid": validdata, "test": testdata}
